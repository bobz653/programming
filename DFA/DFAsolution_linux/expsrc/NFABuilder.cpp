...
CNFABuilder::CNFABuilder()
{
	...
	m_nLen=0;
	m_nDFANo=-1;
	m_mapstruct[ToUni(".",1)]=1;
	m_mapstruct[ToUni("．",2)]=1;
	m_mapstruct[ToUni("·",2)]=1;
	m_mapstruct[ToUni("/",1)]=1;
	m_mapstruct[ToUni("／",2)]=1;
	m_mapstruct[ToUni("‰",2)]=1;
	...
	m_mapwan[ToUni("万",2)]=1;
	m_mapwan[ToUni("亿",2)]=1;
	//电话号码结构单独处理
	m_mapTel[ToUni("e",1)]=1;
	m_mapTel[ToUni("E",1)]=1;
	m_mapTel[ToUni("X",1)]=1;
	m_mapTel[ToUni("x",1)]=1;
	m_mapTel[ToUni("T",1)]=1;
	m_mapTel[ToUni("t",1)]=1;
	m_mapTel[ToUni("(",1)]=1;
	m_mapTel[ToUni(")",1)]=1;
	m_mapTel[ToUni("-",1)]=1;
	m_mapTel[ToUni("－",2)]=1;
	m_mapTel[ToUni("Ｅ",2)]=1;
	m_mapTel[ToUni("ｅ",2)]=1;
	m_mapTel[ToUni("Ｘ",2)]=1;
	m_mapTel[ToUni("ｘ",2)]=1;
	m_mapTel[ToUni("Ｔ",2)]=1;
	m_mapTel[ToUni("ｔ",2)]=1;
	m_mapTel[ToUni("（",2)]=1;
	m_mapTel[ToUni("）",2)]=1;
	
	//时间，年份处理
	m_mapTime[ToUni("时",2)]=1;
	m_mapTime[ToUni("点",2)]=1;
	m_mapTime[ToUni("分",2)]=1;
	m_mapTime[ToUni("秒",2)]=1;
	m_mapTime[ToUni("年",2)]=1;
	m_mapTime[ToUni("月",2)]=1;
	m_mapTime[ToUni("日",2)]=1;
	m_mapTime[ToUni("-",1)]=1;
	m_mapTime[ToUni("－",2)]=1;
	m_mapTime[ToUni("/",1)]=1;
	m_mapTime[ToUni("／",2)]=1;
...
unsigned short CNFABuilder::ToUni(const char* sc,int len)
{
  ...
  int len_uni = bytesToUni(sc, len, wide, 2, "GBK");
	return wide[0];
}
...
//返回-1代表不匹配
int CNFABuilder::Search1(unsigned short *words, char *&sStruct,int &nlen)
{
	return 1;	
}
...
void CNFABuilder::SaveDFAtoFile(char *psFileout)
{
	...
	for(int nCur=0;nCur<m_vecDFANode.size();nCur++){
		...
		int mark=m_vecDFANode[nCur]->bmark;
		fwrite(&mark,sizeof(int),1,fpw);
...
void CNFABuilder::LoadFileToDFA(char *psInfile)
{
  ...
  m_myvec=new PSEARCHNODE[m_myvecnum];
  m_myvecnum=0;
	while(fread(&nNodeNo,sizeof(int),1,fp)!=0){
...
int CNFABuilder::SearchForDFA(unsigned short *words, unsigned char *sStruct, int &nlen,int iLen)
{
	if(m_myvecnum==0)
		return -1;
	int nMtlen=0;
...
//	for(int i=0;i<m_vecStart.size();i++){
		int nCur=0;
		int nMatchlen=0;
		//m_curNo=m_vecReturn[0];
		//int nEdgenum=m_vecStart[i];
		int nEdgenum=0;
		/****start search******/
		//nEdgenum=m_vecSearchNode[nEdgenum]->map_edge[words[nCur++]];
		
		nEdgenum=m_myvec[nEdgenum]->myedge[m_allsym[words[nCur++]]];
		while(nEdgenum!=0&&iLen>=nCur&&nCur<=30){
			if(m_myvec[nEdgenum]->bmark>1){
				nMatchlen=nCur;
				rNo=m_myvec[nEdgenum]->bmark;
			}
			if(iLen<=nCur)
				break;
			nEdgenum=m_myvec[nEdgenum]->myedge[m_allsym[words[nCur++]]];
			//nEdgenum=m_vecSearchNode[nEdgenum]->edge[words[nCur++]];
		//nEdgenum=m_vecSearchNode[nEdgenum]->map_edge[words[nCur++]];
		}
		if(nMatchlen>nlen){
			nlen=nMatchlen;
		...
		if(rNo==JSHU||rNo==IPADD||rNo==EMAIL||rNo==WEB){
			int p=1;
			int nMark=0;
			bool goodend=false;
			for(int k=0;k<nlen;k++){
				map<unsigned short ,int>::iterator myiter=m_mapstruct.find(words[k]);
				if(myiter!=m_mapstruct.end()){
					if(k!=nMark)
						sStruct[p++]=k-nMark;						
					nMark=k+1;
					sStruct[p++]=1;
			...
			if(!goodend){
				if(p!=1){
					sStruct[p++]=nlen-nMark;
				}
			}
			map<unsigned short ,int>::iterator iter=m_mapwan.find(words[nlen-1]);
			if(p!=1&&iter!=m_mapwan.end()){
				sStruct[p-1]-=1;
				sStruct[p++]=1;
			}
			sStruct[p]=0;
			...
			int nMark=0;
			for(int i=0;i<nlen;i++){
				map<unsigned short ,int>::iterator iter=m_mapTel.find(words[i]);
				if(iter!=m_mapTel.end()){
					map<unsigned short ,int>::iterator iterp=m_mapTel.find(words[i+2]);
					if(iterp==m_mapTel.end()){
						if(i!=nMark)
							sStruct[p++]=i-nMark;
						nMark=i+1;
						sStruct[p++]=1;
		...
		}else if(rNo==YMD||rNo==TIMED){
			int p=1;
			int nMark=0;
			bool goodend=false;
			for(int k=0;k<nlen;k++){
				map<unsigned short ,int>::iterator myiter=m_mapTime.find(words[k]);
				if(myiter!=m_mapTime.end()){
					if(k!=nMark)
						sStruct[p++]=k-nMark;						
					nMark=k+1;
					sStruct[p++]=1;
...
void CNFABuilder::releaseDFA(){
	  ...
	  */
	  for(int i=0;i<m_myvecnum;i++){
	  	delete[] m_myvec[i]->myedge;
	  	delete m_myvec[i];
	  }
	  delete[] m_myvec;
	  m_vecStart.clear();
...
int CNFABuilder::Init(const char * filename)
{
		LoadFileToDFA((char*)filename);
}
int CNFABuilder::Search(unsigned short *pStart, int iLen, regnode* retnode)
{
	if(retnode==NULL)
		return -1;
	int nlen=0;
	int nNo=SearchForDFA(pStart, retnode->retstruct,nlen,iLen);
	if(nNo!=-1){
		retnode->type=nNo;
		return nlen;
	}
...
CSegRule * CreateSegRuleInstance(){
	CNFABuilder *myDFA=new CNFABuilder;
	return (CSegRule * )myDFA;
}
int Release(CSegRule *pRule){
	((CNFABuilder*)pRule)->releaseDFA();
	delete pRule;
}
